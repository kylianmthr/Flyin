...
def test_open_file(path: str, expectation) -> None:
    with expectation:
        Parser().open(path)
...
def test_parse_drone_number(line: str, expectation, expected_output) -> None:
    with expectation:
...
def test_parse_start_hub(line: str, expectation, expected_output) -> None:
    with expectation:
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            does_not_raise(),
            {"id": "waypoint", "x": 0, "y": 0, "color": "green", "max_drones": None},
        ),
        (
            "hub: waypoint 0 0",
            does_not_raise(),
            {"id": "waypoint", "x": 0, "y": 0, "color": None, "max_drones": None},
        ),
        (
            "hub: waypoint 0 0 [max_drones=5]",
            does_not_raise(),
            {"id": "waypoint", "x": 0, "y": 0, "color": None, "max_drones": 5},
        ),
        (
            "hub: waypoint 0 0 [color=green max_drones=5]",
            does_not_raise(),
            {"id": "waypoint", "x": 0, "y": 0, "color": "green", "max_drones": 5},
        ),
        ("test", pytest.raises(ValueError), None),
        ("test: r", pytest.raises(ValueError), None),
    ],
)
def test_parse_hub(line: str, expectation, expected_output) -> None:
    with expectation:
...
def test_parse_end_hub(line: str, expectation, expected_output) -> None:
    with expectation:
...
@pytest.mark.parametrize(
    "line, expectation, expected_output",
    [
        ("connection: test-test", does_not_raise(), {"connection": "test-test"}),
        ("test", pytest.raises(ValueError), None),
        ("connection: test", pytest.raises(ValueError), None),
        ("test: r", pytest.raises(ValueError), None),
    ],
)
def test_parse_connection(line: str, expectation, expected_output) -> None:
    with expectation:
